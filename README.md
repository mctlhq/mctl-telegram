...
| `list_telegram_identities`    | `readOnly=true`, `destructive=false`, `openWorld=false` | Admin-only: lists signed-in Telegram identities and access state (first/last name, language code, last-seen, onboarding completion, and per-attribute provenance), with audit metadata. |
