...
CLOUDFLARE_API_TOKEN=… CLOUDFLARE_ACCOUNT_ID=… scripts/portal-allowlist-apply.sh --check
```

The script sends the portal body back exactly as read, minus the read-only timestamps, rewriting only the `tg` mapping — the API overwrites what a PUT omits, and a PUT of the full body was verified to be accepted. Before it reads anything from Cloudflare it re-runs the guard itself: the file must be tracked and match `HEAD`, it must still name `mcp`/`tg`, and `TestPortalAllowlist_CoversEveryRegisteredTool` must report itself passed — `go test -run` exits 0 when its pattern matches nothing, so a renamed or deleted guard is refused rather than skipped. A refusal prints what the test said. What is then applied is the committed blob, not the copy on disk — the two are identical by the check just made, and reading the blob keeps that true through to the PUT. The guard test runs without `CLOUDFLARE_API_TOKEN` in its environment: it is the one step that executes code from the checkout, and previewing an unfamiliar branch should not hand it the portal credential. That narrows the obvious path and is not a trust boundary — a checkout you would not trust with a token is one you should not build. `scripts/portal_allowlist_apply_test.go` drives the script against throwaway checkouts and holds each of those refusals. That covers the gap the test alone leaves — the test gates the repository, the script publishes the working tree, and an edit that enables a mutating tool would otherwise be applied and reported as a clean success before any reviewer saw it. `--dry-run` is held to the same bar, so commit the change first and preview it after. A host without `go`, or a copy of the script outside a checkout, cannot verify the file and is refused rather than trusted, and the file's own `portal`/`server` must still read `mcp`/`tg` — this is the side that writes, and a retargeted file would rewrite a mapping this repository does not own. It refuses to apply when the mapping is missing, when a tool the portal has synced has no decision in the file, and on any API error; the file may list tools the portal has not synced (a deployment on `MCP_TOOL_FILTER=read-only`, or a portal that has not re-synced since a release), and those entries are held back from the write rather than blocking it, and it reads the token from a curl config over a file descriptor so the credential is in neither the process table nor the shell history. CI holds no Cloudflare credential by design (#1111); the apply step is the one manual link, and this section is where it is written down.

`--check` answers the question an apply cannot: whether the portal still agrees with `docs/portal-allowlist.json` right now, without a human opening the dashboard. It compares exactly what an apply would write — `default_disabled`, and the `enabled` state of every tool the server has synced — against what the portal currently has, and it is held to the identical pre-flight as an apply or a `--dry-run`: the same credential, tracked-and-matches-`HEAD`, `mcp`/`tg`, and guard-test checks, so a check against an unreviewed file is refused the same way a write would be. It never issues a `PUT`, on any path, including when it finds disagreement — a check that reports drift changes nothing; re-run the apply to fix it. A file entry for a tool the server has not synced is not drift (the apply holds those back by design) and is named separately as "held back". Exit codes: `0` the portal and the file agree, `1` the comparison could not be made (a guard refused, or an API call failed — nothing was measured, which is not the same as "no drift"), `2` a usage error, `3` the portal and the file disagree. Every non-zero status is a failure a caller must surface: the split between `1` and `3` exists to tell a human which of the two happened, never to let a caller treat one of them as tolerable. A job that alerts on `3` alone reports "no drift" when the token has expired, the scope was revoked or the API was down — the same answer a working check gives when everything agrees, and the same one a broken check gives. Wiring `--check` into a scheduled CI job is mctlhq/mctl-gitops#1211 and is not part of this change; today it is operator-run, same as the apply.
