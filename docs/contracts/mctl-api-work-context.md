...
**Pinned copy, not the source of truth.** This file summarises the mctl-api contracts the Telegram work-item adapter (#443) must build against. The authoritative text lives in mctl-api at revision `10bc4cd` (main, 2026-09-23):

- `docs/work-context-contract.md`: § Identity and authorization, § REST surface, § Implementation status → Surface relay;
...
4. **Execution attachment is platform-owned.** The bot never starts, attaches or correlates an execution, and never seals or reads a context snapshot. Those routes are service-principal only (the engine side: mctl-agents / Temporal). The bot records user intent and asks for a resume. The platform runs the execution.
...
| `POST /api/v1/work-items/{id}/resume` | `{expected_state_version, resumed_from_execution_id?, engine, engine_ref, surface?, idempotency_key?}` | Asks the platform to continue. |
