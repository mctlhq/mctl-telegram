...
// succeeded, and no fatal auth error has occurred since. This is the
// /readyz signal — false during startup (before the first poll completes),
// after a transient poll failure (until the next successful one), or
// permanently after a fatal auth error.
func (h *Health) Ready() bool {
	if h == nil {
		return true
	}
	return h.polledOnce.Load() && h.pollOK.Load() && !h.fatal.Load()
}
