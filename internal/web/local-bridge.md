...
- `send_media` `file_path` is Local Bridge only, and only on Linux and
  macOS daemons. Windows has no secure fd-to-path check, so `file_path`
  is refused there; use `file_base64`. The daemon reads only files
